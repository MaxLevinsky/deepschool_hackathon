...
#from ultralytics.model import TASK_MAP
...
def train_v2(self: YOLO, pruning=False, **kwargs):
    ...
    self.trainer = DetectionTrainer(overrides=overrides, _callbacks=self.callbacks)
...
def prune(args, data_yml, epochs = 10):
    ...
    LOGGER.info(f"validation strats...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='./1000g_agu_small_nofm.pt', help='Pretrained pruning target model file')
    parser.add_argument('--cfg', default=cfg_yml,
    ...
    parser.add_argument('--target-prune-rate', default=0.4, type=float, help='Target pruning rate') # сколько выкидываем
    parser.add_argument('--max-map-drop', default=0.05, type=float, help='Allowed maximum map drop after fine-tuning')

    args = parser.parse_args()


    prune(args, data_yml = "./data.yaml", epochs=100)
